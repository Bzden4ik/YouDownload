...
function secsToTimestamp(s) {
  const h = Math.floor(s / 3600), m2 = Math.floor(s % 3600 / 60), sec = s % 60;
  return `${String(h).padStart(2, "0")}:${String(m2).padStart(2, "0")}:${String(sec).padStart(2, "0")}`;
}
function parseTimestamp(raw) {
  const s = raw.trim();
  if (!s) return null;
  const parts = s.split(":").map((p2) => parseFloat(p2));
  if (parts.some((p2) => isNaN(p2))) return null;
...
function TimeRangePicker({ duration, startSec, endSec, onChange }) {
  ...
  const pct = (v2) => Math.max(0, Math.min(100, v2 / duration * 100));
  ...
  const startDrag = (handle) => (e) => {
    ...
    const onMove = (me2) => {
      const sec = posToSec(me2.clientX);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tr-slider-wrap", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tr-slider-meta", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "tr-sel-label", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { width: "11", height: "11", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2.2", children: /* @__PURE__ */ jsxRuntimeExports.jsx("polygon", { points: "5 3 19 12 5 21 5 3" }) }),
        secsToTimestamp(startSec)
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "tr-sel-dur", children: secsToTimestamp(selDur) }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "tr-sel-label", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { width: "11", height: "11", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2.2", children: /* @__PURE__ */ jsxRuntimeExports.jsx("rect", { x: "3", y: "3", width: "18", height: "18", rx: "2" }) }),
        secsToTimestamp(endSec)
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tr-track-outer", ref: trackRef, onClick: clickTrack, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tr-track-bg" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tr-track-fill", style: { left: `${startPct}%`, width: `${endPct - startPct}%` } }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tr-handle tr-handle-s", style: { left: `${startPct}%` }, onMouseDown: startDrag("start"), children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tr-handle-pip" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tr-handle-label tr-label-top", children: secsToTimestamp(startSec) })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tr-handle tr-handle-e", style: { left: `${endPct}%` }, onMouseDown: startDrag("end"), children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tr-handle-pip" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tr-handle-label tr-label-top", children: secsToTimestamp(endSec) })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tr-edge-labels", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "00:00:00" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: secsToTimestamp(duration) })
    ] })
  ] });
}
function extractEmbedId(url, platform) {
  if (platform === "twitch") {
    const m22 = url.match(/twitch\.tv\/videos?\/([0-9]+)/);
    return m22?.[1] ?? null;
  }
  const m2 = url.match(/(?:v=|youtu\.be\/|embed\/)([a-zA-Z0-9_-]{11})/);
  return m2?.[1] ?? null;
}
function VideoPlayerPanel({ url, platform, duration, onDownload, onClose, t: t2 }) {
  ...
  const [ftype, setFtype] = reactExports.useState("video");
  const [embedUrl, setEmbedUrl] = reactExports.useState(null);
  const [manualStart, setManualStart] = reactExports.useState("");
  const [manualEnd, setManualEnd] = reactExports.useState("");
  const [manualSeek, setManualSeek] = reactExports.useState("");
  const [manualStartErr, setManualStartErr] = reactExports.useState(false);
  const [manualEndErr, setManualEndErr] = reactExports.useState(false);
  const [manualSeekErr, setManualSeekErr] = reactExports.useState(false);
  const formatTimeInput = (prev, next) => {
    if (next.length < prev.replace(/:/g, "").length) {
      const digits2 = prev.replace(/:/g, "");
      const shorter = digits2.slice(0, -1);
  ...
  reactExports.useEffect(() => {
    if (platform === "twitch") {
      ...
      window.api.getPreviewPort().then((port) => {
        setEmbedUrl(`http://localhost:${port}/?id=${id2}`);
      }).catch(() => {
        setEmbedUrl(`https://player.twitch.tv/?video=${id2}&parent=localhost&autoplay=false`);
      });
    } else {
      const id2 = extractEmbedId(url, platform);
      if (!id2) {
  ...
  const applyManualStart = () => {
    const v2 = parseTimestamp(manualStart);
    if (v2 === null || v2 < 0) {
      setManualStartErr(true);
      return;
    }
    setManualStartErr(false);
    setManualStart("");
    const clamped = Math.max(0, Math.min(v2, (dur > 0 ? dur : 999999) - 1));
    setStartSec(clamped);
    if (endSec <= clamped) setEndSec(Math.min(clamped + 30, dur > 0 ? dur : clamped + 30));
  };
  const applyManualEnd = () => {
    const v2 = parseTimestamp(manualEnd);
    if (v2 === null || v2 <= 0) {
      setManualEndErr(true);
      return;
    }
    setManualEndErr(false);
    setManualEnd("");
    const clamped = dur > 0 ? Math.min(v2, dur) : v2;
    setEndSec(clamped);
    if (startSec >= clamped) setStartSec(Math.max(0, clamped - 30));
  };
  const applyManualSeek = () => {
    const v2 = parseTimestamp(manualSeek);
    if (v2 === null || v2 < 0) {
      setManualSeekErr(true);
      return;
    }
  ...
  const pct = (v2) => dur > 0 ? Math.max(0, Math.min(100, v2 / dur * 100)) : 0;
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "vp-panel", ref: panelRef, style: panelStyle, children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "vp-head", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "vp-cur", children: secsToTimestamp(currentTime) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "vp-close", onClick: onClose, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { width: "13", height: "13", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("line", { x1: "18", y1: "6", x2: "6", y2: "18" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("line", { x1: "6", y1: "6", x2: "18", y2: "18" })
      ] }) })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "vp-player-wrap", children: [
      !ready && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "vp-loading", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "spin" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: t2.vp_loading })
      ] }),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "vp-timeline", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "vp-tl-info", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "vp-play-ctrl", onClick: togglePlay, disabled: !ready, title: playing ? "Pause" : "Play", children: playing ? /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { width: "11", height: "11", viewBox: "0 0 24 24", fill: "currentColor", stroke: "none", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("rect", { x: "5", y: "3", width: "4", height: "18", rx: "1" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("rect", { x: "15", y: "3", width: "4", height: "18", rx: "1" })
        ] }) : /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { width: "11", height: "11", viewBox: "0 0 24 24", fill: "currentColor", stroke: "none", children: /* @__PURE__ */ jsxRuntimeExports.jsx("polygon", { points: "5 3 19 12 5 21 5 3" }) }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "vp-tl-cur", children: secsToTimestamp(currentTime) }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "vp-tl-sel-info", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "vp-ts", onClick: () => seekTo(startSec), title: t2.vp_seek_hint, children: secsToTimestamp(startSec) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "vp-tl-arrow", children: "→" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "vp-ts", onClick: () => seekTo(endSec), title: t2.vp_seek_hint, children: secsToTimestamp(endSec) }),
          dur > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "vp-sel-dur", children: secsToTimestamp(selectedDur) })
        ] }),
        dur > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "vp-tl-total", children: secsToTimestamp(dur) })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "vp-tl-track", ref: trackRef, onClick: clickTrack, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "vp-tl-bg" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "vp-tl-sel", style: { left: `${pct(startSec)}%`, width: `${pct(endSec) - pct(startSec)}%` } }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "vp-tl-head", style: { left: `${pct(currentTime)}%` }, onMouseDown: startDrag("head") }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "vp-tl-handle vp-tl-handle-s", style: { left: `${pct(startSec)}%` }, onMouseDown: startDrag("start"), onClick: (e) => e.stopPropagation(), children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "vp-tl-handle-pip vp-pip-s" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "vp-tl-handle-label", children: secsToTimestamp(startSec) })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "vp-tl-handle vp-tl-handle-e", style: { left: `${pct(endSec)}%` }, onMouseDown: startDrag("end"), onClick: (e) => e.stopPropagation(), children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "vp-tl-handle-pip vp-pip-e" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "vp-tl-handle-label vp-label-right", children: secsToTimestamp(endSec) })
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "vp-tl-edge-labels", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "00:00:00" }),
        dur > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: secsToTimestamp(dur) })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "vp-footer", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "vp-button-flow transparent", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "vp-fmt-row-buttonBack", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "vp-mark-btn vp-mark-s", onClick: markStart, disabled: !ready, children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { width: "11", height: "11", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2.2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("rect", { x: "4", y: "3", width: "3", height: "18", rx: "1", fill: "currentColor", stroke: "none" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("line", { x1: "7", y1: "12", x2: "20", y2: "12" })
          ] }),
          t2.vp_mark_start
        ] }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "vp-footer-row2", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "vp-time-group", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "vp-time-label", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { width: "9", height: "9", viewBox: "0 0 24 24", fill: "currentColor", stroke: "none", children: /* @__PURE__ */ jsxRuntimeExports.jsx("rect", { x: "4", y: "3", width: "3", height: "18", rx: "1" }) }),
              t2.vp_input_start
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `vp-time-field ${manualStartErr ? "vp-time-err" : ""}`, children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "input",
                {
                  ...
                  value: manualStart,
                  onChange: (e) => {
                    setManualStart((v2) => formatTimeInput(v2, e.target.value));
                    setManualStartErr(false);
                  },
                  onKeyDown: (e) => e.key === "Enter" && applyManualStart()
                }
              ),
              /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "vp-time-apply", onClick: applyManualStart, title: "Apply", children: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { width: "10", height: "10", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2.5", children: /* @__PURE__ */ jsxRuntimeExports.jsx("polyline", { points: "20 6 9 17 4 12" }) }) })
            ] })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "vp-time-group", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "vp-time-label", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { width: "9", height: "9", viewBox: "0 0 24 24", fill: "currentColor", stroke: "none", children: /* @__PURE__ */ jsxRuntimeExports.jsx("rect", { x: "17", y: "3", width: "3", height: "18", rx: "1" }) }),
              t2.vp_input_end
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `vp-time-field ${manualEndErr ? "vp-time-err" : ""}`, children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "input",
                {
                  className: "vp-time-input",
                  ...
                  value: manualEnd,
                  onChange: (e) => {
                    setManualEnd((v2) => formatTimeInput(v2, e.target.value));
                    setManualEndErr(false);
                  },
                  onKeyDown: (e) => e.key === "Enter" && applyManualEnd()
                }
              ),
              /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "vp-time-apply", onClick: applyManualEnd, title: "Apply", children: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { width: "10", height: "10", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2.5", children: /* @__PURE__ */ jsxRuntimeExports.jsx("polyline", { points: "20 6 9 17 4 12" }) }) })
            ] })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "vp-time-group", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "vp-time-label", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { width: "9", height: "9", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2.5", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("circle", { cx: "12", cy: "12", r: "9" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("polyline", { points: "12 7 12 12 15 15" })
              ] }),
              t2.vp_input_seek
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `vp-time-field ${manualSeekErr ? "vp-time-err" : ""}`, children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "input",
                {
                  className: "vp-time-input",
                  ...
                  value: manualSeek,
                  onChange: (e) => {
                    setManualSeek((v2) => formatTimeInput(v2, e.target.value));
                    setManualSeekErr(false);
                  },
                  onKeyDown: (e) => e.key === "Enter" && applyManualSeek()
                }
              ),
              /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "vp-time-apply", onClick: applyManualSeek, title: "Go", children: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { width: "10", height: "10", viewBox: "0 0 24 24", fill: "currentColor", stroke: "none", children: /* @__PURE__ */ jsxRuntimeExports.jsx("polygon", { points: "5 3 19 12 5 21 5 3" }) }) })
            ] })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "vp-time-hint", children: "HH:MM:SS" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "vp-fmt-row-buttonBack", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "vp-mark-btn vp-mark-e", onClick: markEnd, disabled: !ready, children: [
          t2.vp_mark_end,
          /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { width: "11", height: "11", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2.2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("rect", { x: "17", y: "3", width: "3", height: "18", rx: "1", fill: "currentColor", stroke: "none" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("line", { x1: "4", y1: "12", x2: "17", y2: "12" })
          ] })
        ] }) })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "vp-fmt-pow", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "vp-fmt-row", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: `vp-fmt-tab ${ftype === "video" ? "vp-fmt-on" : ""}`, onClick: () => setFtype("video"), children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: `vp-fmt-tab ${ftype === "audio" ? "vp-fmt-on" : ""}`, onClick: () => setFtype("audio"), children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { width: "11", height: "11", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "vp-fmt-row-buttonBack", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
          className: "vp-dl-btn",
          disabled: selectedDur <= 0,
          ...
          children: [
            ...
            t2.vp_download,
            /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "vp-dl-range", children: [
              secsToTimestamp(startSec),
              " → ",
              secsToTimestamp(endSec)
            ] })
...
function StarLogo({ size = 32, cls = "" }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { width: size, height: size, viewBox: "0 0 32 32", className: cls, fill: "none", xmlns: "http://www.w3.org/2000/svg", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("circle", { cx: "16", cy: "16", r: "14.5", stroke: "currentColor", strokeWidth: "0.8", opacity: "0.35" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("circle", { cx: "16", cy: "16", r: "11", stroke: "currentColor", strokeWidth: "0.4", opacity: "0.2" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("path", { d: "M16 1.5 L17.8 14.2 L30.5 16 L17.8 17.8 L16 30.5 L14.2 17.8 L1.5 16 L14.2 14.2 Z", fill: "currentColor" })
  ] });
}
function TitleBar() {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "titlebar", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "titlebar-drag", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(StarLogo, { size: 14, cls: "tb-star" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "tb-name", children: "YouDownload" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "titlebar-controls", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "wc-btn wc-min", onClick: () => window.api?.minimize(), "aria-label": "Minimize", children: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { width: "11", height: "1", viewBox: "0 0 11 1", children: /* @__PURE__ */ jsxRuntimeExports.jsx("rect", { width: "11", height: "1.2", fill: "currentColor" }) }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "wc-btn wc-max", onClick: () => window.api?.maximize(), "aria-label": "Maximize", children: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { width: "11", height: "11", viewBox: "0 0 11 11", children: /* @__PURE__ */ jsxRuntimeExports.jsx("rect", { x: ".5", y: ".5", width: "10", height: "10", stroke: "currentColor", strokeWidth: "1.1", fill: "none" }) }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "wc-btn wc-close", onClick: () => window.api?.close(), "aria-label": "Close", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { width: "11", height: "11", viewBox: "0 0 11 11", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("line", { x1: ".5", y1: ".5", x2: "10.5", y2: "10.5", stroke: "currentColor", strokeWidth: "1.2" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("line", { x1: "10.5", y1: ".5", x2: ".5", y2: "10.5", stroke: "currentColor", strokeWidth: "1.2" })
      ] }) })
    ] })
  ] });
}
function Sidebar({ view, onChange, activeCount, lang, onLangToggle, collapsed, onToggleCollapse }) {
  ...
  const NAV = [
    ...
    { id: "stream", label: t2.nav_stream, icon: /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { width: "17", height: "17", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "1.6", children: [
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("aside", { className: `sidebar${collapsed ? " sb-collapsed" : ""}`, children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "sb-footer", children: [
      ...
      !collapsed && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "sb-version", children: "v1.1.4" })
    ] })
  ] });
}
function normalizeTwitchInput(val) {
  const v2 = val.trim();
  if (!v2) return v2;
  if (v2.startsWith("http://") || v2.startsWith("https://")) return v2;
  if (v2.includes(".")) return `https://${v2}`;
  return `https://www.twitch.tv/${v2}`;
}
function UrlInput({ onFetch, loading, t: t2, platform, onPlatformChange }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "url-section", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "platform-switcher", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: `platform-btn ${platform === "vk" ? "platform-btn-on platform-btn-vk" : ""}`, onClick: () => onPlatformChange("vk"), children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { width: "13", height: "13", viewBox: "0 0 24 24", fill: "currentColor", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { d: "M21.3 0H2.7C1.2 0 0 1.2 0 2.7v18.6C0 22.8 1.2 24 2.7 24h18.6c1.5 0 2.7-1.2 2.7-2.7V2.7C24 1.2 22.8 0 21.3 0zm-1.6 16.9h-2c-.8 0-1-.6-2.3-1.9-1.1-1.2-1.6-1.3-1.9-1.3-.4 0-.5.1-.5.7v1.7c0 .5-.2.8-1.4.8-2 0-4.2-1.2-5.8-3.5C4.3 10.6 3.6 8 3.6 7.5c0-.3.1-.5.7-.5h2c.5 0 .7.2.9.8.9 2.5 2.5 4.7 3.1 4.7.2 0 .3-.1.3-.7V9c-.1-1.5-.9-1.6-.9-2.1 0-.3.2-.5.6-.5h3.1c.4 0 .6.2.6.8v3.5c0 .4.2.6.3.6.2 0 .4-.1.8-.5 1.3-1.5 2.2-3.7 2.2-3.7.1-.3.4-.6.9-.6h2c.6 0 .7.3.6.8-.3 1.2-2.8 4.8-2.8 4.8-.2.3-.3.5 0 .9.2.3.9.9 1.4 1.5.9.9 1.5 1.7 1.7 2.2.2.5-.1.8-.6.8z" }) }),
        "VK"
      ] })
    ] }),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "url-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "url-icon", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { width: "15", height: "15", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "1.5", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "btn-paste", onClick: pasteFromClipboard, children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: t2.btn_paste })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn-fetch", onClick: submit, disabled: !url.trim() || loading, children: loading ? /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "spin" }) : /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "url-hint", children: platform === "twitch" ? t2.twitch_hint : platform === "vk" ? t2.vk_hint : t2.url_hint })
  ] });
}
function VideoInfoCard({ info, loading, t: t2 }) {
  if (loading) return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "vi-card vi-skeleton", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "skel-thumb" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "skel-body", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "skel-line w80" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "skel-line w55" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "skel-line w35" })
    ] })
  ] });
  if (!info) return null;
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "vi-card", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "vi-thumb-wrap", children: [
      info.thumbnail ? /* @__PURE__ */ jsxRuntimeExports.jsx("img", { src: info.thumbnail, alt: "", className: "vi-thumb" }) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "vi-thumb-ph", children: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { width: "28", height: "28", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "1", children: /* @__PURE__ */ jsxRuntimeExports.jsx("polygon", { points: "5 3 19 12 5 21 5 3" }) }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "vi-dur", children: formatDur(info.duration) })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "vi-meta", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "vi-title", children: info.title }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "vi-stats", children: [
        info.uploader && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "vi-channel", children: [
          "@",
          info.uploader
        ] }),
        info.view_count ? /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "vi-dot", children: "·" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "vi-views", children: formatViews(info.view_count, t2) })
        ] }) : null
      ] })
    ] })
  ] });
}
function FormatSelector({ onDownload, onDownloadAll, disabled, t: t2, initType, initVq, initAq, onFormatChange, playlist, platform, availableQualities, duration, onOpenPlayer }) {
  ...
  const [useTimeRange, setUseTimeRange] = reactExports.useState(false);
  const [trStart, setTrStart] = reactExports.useState(0);
  const [trEnd, setTrEnd] = reactExports.useState(duration ?? 0);
  reactExports.useEffect(() => {
  ...
  const setVqAndSave = (v2) => {
    setVq(v2);
    onFormatChange(ftype, v2, aq);
  };
  const setAqAndSave = (v2) => {
    setAq(v2);
  ...
  const go = () => {
    const timeRange = useTimeRange && isTwitch && duration && duration > 0 ? { start: trStart, end: trEnd >= duration ? -1 : trEnd } : void 0;
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "fmt-section", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "fmt-tabs", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: `fmt-tab ${ftype === "audio" ? "fmt-tab-on" : ""}`, onClick: () => setFtypeAndSave("audio"), children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { width: "14", height: "14", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "1.5", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("path", { d: "M9 18V5l12-2v13" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("circle", { cx: "6", cy: "18", r: "3" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("circle", { cx: "18", cy: "16", r: "3" })
    ...
    (platform === "twitch" || platform === "youtube") && onOpenPlayer && /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "vp-open-btn", onClick: onOpenPlayer, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { width: "13", height: "13", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: /* @__PURE__ */ jsxRuntimeExports.jsx("polygon", { points: "5 3 19 12 5 21 5 3" }) }),
      t2.vp_open_player
    ] }),
    isTwitch && !onOpenPlayer && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "time-range-section", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "time-range-header", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: `time-range-toggle ${useTimeRange ? "time-range-toggle-on" : ""}`, onClick: () => setUseTimeRange((v2) => !v2), children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { width: "13", height: "13", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "1.8", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("circle", { cx: "12", cy: "12", r: "9" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("polyline", { points: "12 7 12 12 15 15" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: t2.time_range_toggle }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: `tr-arrow${useTimeRange ? " tr-arrow-open" : ""}`, width: "11", height: "11", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2.5", children: /* @__PURE__ */ jsxRuntimeExports.jsx("polyline", { points: "6 9 12 15 18 9" }) })
      ] }) }),
      useTimeRange && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "time-range-body", children: duration && duration > 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx(TimeRangePicker, { duration, startSec: trStart, endSec: trEnd, onChange: (s, e) => {
        setTrStart(s);
        setTrEnd(e);
      } }) : /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "tr-no-dur", children: t2.time_range_no_duration }) })
    ] }),
...
function TwitchChannelBrowser({ channelName, t: t2, onSelect, onDownloadMulti, onPinToggle, onBack }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "twitch-browser", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "twitch-browser-head", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "twitch-open-btn", onClick: () => window.api?.openExternal(`https://www.twitch.tv/${channelName}/videos`), title: "Open on Twitch", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "twitch-search", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { width: "13", height: "13", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("circle", { cx: "11", cy: "11", r: "8" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("line", { x1: "21", y1: "21", x2: "16.65", y2: "16.65" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
          ...
          type: "text",
          placeholder: t2.twitch_search_placeholder,
          value: searchQuery,
          onChange: (e) => setSearchQuery(e.target.value)
        }
      ),
      searchQuery && /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "twitch-search-clear", onClick: () => setSearchQuery(""), children: /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { width: "11", height: "11", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("line", { x1: "18", y1: "6", x2: "6", y2: "18" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("line", { x1: "6", y1: "6", x2: "18", y2: "18" })
      ] }) })
    ] }),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "twitch-list", children: [
      ...
      sortedEntries.map((entry) => {
        ...
        return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `twitch-entry ${isSelected ? "twitch-entry-selected" : ""}`, children: [
          ...
          }, title: "Watch on Twitch", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { width: "12", height: "12", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("path", { d: "M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("polyline", { points: "15 3 21 3 21 9" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("line", { x1: "10", y1: "14", x2: "21", y2: "3" })
          ] }) })
...
function DownloadCard({ item, onCancel, onOpen, onCookieHint, onFfmpegHint, t: t2, lang }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `dl-card dl-${item.status}`, children: [
    item.thumbnail && /* @__PURE__ */ jsxRuntimeExports.jsx("img", { src: item.thumbnail, alt: "", className: "dl-thumb" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dl-body", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dl-top", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dl-title", title: item.title, children: item.title }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dl-acts", children: [
          (item.status === "downloading" || item.status === "pending") && /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "dl-act", onClick: () => onCancel(item.id), title: "Cancel", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { width: "12", height: "12", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("circle", { cx: "12", cy: "12", r: "9" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("line", { x1: "15", y1: "9", x2: "9", y2: "15" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("line", { x1: "9", y1: "9", x2: "15", y2: "15" })
          ] }) }),
          item.status === "complete" && /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "dl-act dl-act-open", onClick: () => onOpen(item.id), title: "Open folder", children: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { width: "12", height: "12", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { d: "M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z" }) }) })
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dl-info-row", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dl-fmt", children: item.formatLabel }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "dl-status-tag", style: { color: c }, children: [
        ...
        item.status === "downloading" && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "dl-pct", children: [
          Math.round(item.progress),
          "%"
        ] })
      ] }),
      showBar && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dl-track", children: [
      ...
      item.error && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dl-err-wrap", children: [
        ...
        isFfmpegError(item.error) && /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "dl-cookie-hint dl-ffmpeg-hint", onClick: onFfmpegHint, children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { width: "11", height: "11", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { d: "M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" }) }),
          t2.err_ffmpeg_install ?? "Install ffmpeg"
        ] })
      ] })
    ] })
  ] });
}
function HistoryView({ downloads, t: t2, onClear }) {
  const done = downloads.filter((d) => ["complete", "error", "cancelled"].includes(d.status));
  if (!done.length) return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "hist-empty", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(StarLogo, { size: 52, cls: "hist-empty-star" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: t2.hist_empty })
  ] });
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "hist-view", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "hist-head", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "section-title", children: t2.hist_title }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "hist-count", children: [
        done.length,
        " ",
        t2.hist_items
      ] }),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "hist-list", children: done.map((d) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `hist-item hist-${d.status}`, children: [
      d.thumbnail && /* @__PURE__ */ jsxRuntimeExports.jsx("img", { src: d.thumbnail, alt: "", className: "hist-thumb" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "hist-meta", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "hist-title", children: d.title }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "hist-fmt", children: d.formatLabel })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "hist-icon", style: { color: STATUS_COLOR[d.status] }, children: d.status === "complete" ? "✓" : d.status === "error" ? "✗" : "–" })
    ] }, d.id)) })
  ] });
}
function GlowOrbs() {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "ap-orbs", "aria-hidden": true, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "ap-orb ap-orb-1" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "ap-orb ap-orb-2" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "ap-orb ap-orb-3" })
  ] });
}
function ThemeCards({ current, onChange, t: t2 }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "theme-cards", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: `theme-card ${current === "fleet" ? "theme-card-on" : ""}`, onClick: () => onChange("fleet"), children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tc-preview tc-fleet", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tc-fleet-sb", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tc-fleet-dot" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tc-fleet-line active" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tc-fleet-line" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tc-fleet-line" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tc-fleet-main", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tc-fleet-bar", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tc-fleet-bar-fill" }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tc-fleet-accent" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tc-fleet-bar", style: { width: "70%" }, children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tc-fleet-bar-fill", style: { width: "35%", background: "#38BDF8" } }) })
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tc-footer", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "tc-name", children: t2.theme_fleet }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "tc-check", children: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { viewBox: "0 0 10 8", fill: "none", children: /* @__PURE__ */ jsxRuntimeExports.jsx("polyline", { points: "1 4 3.5 6.5 9 1", stroke: "#000", strokeWidth: "1.5", strokeLinecap: "round" }) }) })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: `theme-card ${current === "apathy" ? "theme-card-on" : ""}`, onClick: () => onChange("apathy"), children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tc-preview tc-apathy", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tc-ap-orb1" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tc-ap-orb2" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tc-ap-sb", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tc-ap-dot" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tc-ap-line active" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tc-ap-line" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tc-ap-line" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tc-ap-main", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tc-ap-bar", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tc-ap-bar-fill" }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tc-ap-accent" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tc-ap-bar", style: { width: "70%" }, children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tc-ap-bar-fill", style: { width: "35%", background: "#4D7CFF" } }) })
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tc-footer", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "tc-name", children: t2.theme_apathy }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "tc-check", children: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { viewBox: "0 0 10 8", fill: "none", children: /* @__PURE__ */ jsxRuntimeExports.jsx("polyline", { points: "1 4 3.5 6.5 9 1", stroke: "#000", strokeWidth: "1.5", strokeLinecap: "round" }) }) })
      ] })
    ] })
  ] });
...
function SettingsView({ settings, onSave, onPickFolder, t: t2, theme, onThemeChange, highlightCookies, autoCheckUpdates, onAutoCheckChange, onManualCheck, twitchLoggedIn, twitchExtractState, twitchExtractError, onTwitchExtract }) {
  ...
  const [vkExtractState, setVkExtractState] = reactExports.useState("idle");
  const [vkExtractError, setVkExtractError] = reactExports.useState("");
  const [vkLoggedIn, setVkLoggedIn] = reactExports.useState(false);
  ...
  reactExports.useEffect(() => {
    window.api?.checkVkSession().then((r2) => setVkLoggedIn(r2.loggedIn)).catch(() => {
    });
  }, []);
  reactExports.useEffect(() => {
  ...
  const handleVkExtract = async () => {
    setVkExtractState("busy");
    setVkExtractError("");
    const r2 = await window.api?.extractVkCookies();
    if (r2?.success) {
      setVkExtractState("ok");
      setVkLoggedIn(true);
    } else {
      setVkExtractState("fail");
      setVkExtractError(r2?.error || "");
    }
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "settings-view", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "set-group", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "set-label", children: t2.set_theme }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(ThemeCards, { current: theme, onChange: onThemeChange, t: t2 })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "set-group", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "set-label", children: t2.set_folder_label }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "set-row", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("input", { className: "set-input", value: local.downloadPath, readOnly: true, onChange: () => {
        } }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "set-browse", onClick: onPickFolder, children: t2.set_folder_browse })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "set-group", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "set-label", children: t2.set_concurrent }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "set-radios", children: [
        [1, 2, 3, 5, 10, 20].map((n2) => /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: `set-radio ${local.concurrentDownloads === n2 ? "set-radio-on" : ""}`, onClick: () => setLocal((p2) => ({ ...p2, concurrentDownloads: n2 })), children: n2 }, n2)),
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: `set-radio ${local.concurrentDownloads === 0 ? "set-radio-on" : ""}`, onClick: () => setLocal((p2) => ({ ...p2, concurrentDownloads: 0 })), children: "∞" })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "set-group", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "set-group", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "set-label", children: t2.set_extract_vk_cookies }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "set-hint", children: t2.set_extract_vk_cookies_hint }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: `set-extract-btn ${vkExtractState === "ok" ? "set-extract-ok" : vkExtractState === "fail" ? "set-extract-fail" : ""}`, onClick: handleVkExtract, disabled: vkExtractState === "busy", children: vkExtractState === "busy" ? /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "spin" }),
        t2.set_extracting
      ] }) : vkExtractState === "ok" ? /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { width: "13", height: "13", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2.5", children: /* @__PURE__ */ jsxRuntimeExports.jsx("polyline", { points: "20 6 9 17 4 12" }) }),
        t2.set_extract_vk_ok
      ] }) : vkExtractState === "fail" ? /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { width: "13", height: "13", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("circle", { cx: "12", cy: "12", r: "10" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("line", { x1: "12", y1: "8", x2: "12", y2: "12" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("line", { x1: "12", y1: "16", x2: "12.01", y2: "16" })
        ] }),
        t2.set_extract_vk_fail
      ] }) : /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { width: "13", height: "13", viewBox: "0 0 24 24", fill: "currentColor", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { d: "M21.3 0H2.7C1.2 0 0 1.2 0 2.7v18.6C0 22.8 1.2 24 2.7 24h18.6c1.5 0 2.7-1.2 2.7-2.7V2.7C24 1.2 22.8 0 21.3 0zm-1.6 16.9h-2c-.8 0-1-.6-2.3-1.9-1.1-1.2-1.6-1.3-1.9-1.3-.4 0-.5.1-.5.7v1.7c0 .5-.2.8-1.4.8-2 0-4.2-1.2-5.8-3.5C4.3 10.6 3.6 8 3.6 7.5c0-.3.1-.5.7-.5h2c.5 0 .7.2.9.8.9 2.5 2.5 4.7 3.1 4.7.2 0 .3-.1.3-.7V9c-.1-1.5-.9-1.6-.9-2.1 0-.3.2-.5.6-.5h3.1c.4 0 .6.2.6.8v3.5c0 .4.2.6.3.6.2 0 .4-.1.8-.5 1.3-1.5 2.2-3.7 2.2-3.7.1-.3.4-.6.9-.6h2c.6 0 .7.3.6.8-.3 1.2-2.8 4.8-2.8 4.8-.2.3-.3.5 0 .9.2.3.9.9 1.4 1.5.9.9 1.5 1.7 1.7 2.2.2.5-.1.8-.6.8z" }) }),
        t2.set_extract_vk_cookies,
...
function SetupOverlay({ onSetup, loading, error, t: t2 }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "setup-overlay", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "setup-card", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(StarLogo, { size: 72, cls: "setup-star" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { className: "setup-name", children: "YouDownload" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "setup-sub", children: t2.setup_sub }),
    error && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "setup-err", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { width: "14", height: "14", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("circle", { cx: "12", cy: "12", r: "10" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("line", { x1: "12", y1: "8", x2: "12", y2: "12" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("line", { x1: "12", y1: "16", x2: "12.01", y2: "16" })
    ...
    loading ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "setup-loading", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "setup-spin" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: t2.setup_loading })
    ] }) : /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "setup-btns", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "setup-btn", onClick: onSetup, children: error ? t2.setup_retry : t2.setup_init }),
      error && /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "setup-manual", children: [
        t2.setup_manual,
        /* @__PURE__ */ jsxRuntimeExports.jsx("br", {}),
        /* @__PURE__ */ jsxRuntimeExports.jsx("code", { children: "winget install yt-dlp" })
      ] })
    ] })
  ] }) });
}
...
function normalizeTwitchChannel(val) {
  ...
  if (!v2) return "";
  try {
    const u2 = new URL(v2.startsWith("http") ? v2 : `https://${v2}`);
    if (u2.hostname.includes("twitch.tv")) {
...
function StreamView({ t: t2, downloadPath, persistedInput, persistedChannelName, persistedMarkers, streamSessions, hidden, onInputChange, onChannelChange, onMarkersChange, onSessionsChange, onStartDownload }) {
  ...
  reactExports.useEffect(() => {
    ...
    const fallback = setTimeout(() => setPlayerReady(true), 8e3);
    wv.addEventListener("dom-ready", onReady);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "stream-view", style: hidden ? { display: "none" } : void 0, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "stream-input-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "stream-input-wrap", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "input",
          {
            ...
            type: "text",
            placeholder: t2.stream_placeholder,
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "stream-watch-btn", onClick: handleWatch, disabled: !persistedInput.trim(), children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { width: "13", height: "13", viewBox: "0 0 24 24", fill: "currentColor", stroke: "none", children: /* @__PURE__ */ jsxRuntimeExports.jsx("polygon", { points: "5 3 19 12 5 21 5 3" }) }),
        t2.stream_watch
      ] })
    ] }),
    ...
    streamTab === "live" && persistedChannelName && embedUrl && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "stream-body", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "stream-player-wrap", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "stream-webview-wrap", children: [
          !playerReady && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "stream-player-loading", children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "spin" }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "webview",
            {
              ref: webviewRef,
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "stream-markers-panel", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "stream-markers-head", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "stream-markers-title", children: t2.stream_markers_title }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "stream-markers-count", children: persistedMarkers.length })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "stream-presets", children: PRESETS.map((label) => /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "stream-preset-btn", onClick: () => addPresetMarker(label), disabled: !playerReady, children: label }, label)) }),
        !showAddForm ? /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "stream-add-marker-btn", onClick: () => setShowAddForm(true), disabled: !playerReady, children: [
          ...
          t2.stream_add_marker
        ] }) : /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "stream-marker-form", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "smf-pos", children: [
            t2.stream_at,
            " ",
            secsToTimestamp(streamPos)
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("input", { className: "smf-input", placeholder: t2.stream_marker_name, value: newName, onChange: (e) => setNewName(e.target.value), onKeyDown: (e) => e.key === "Enter" && addMarker(), autoFocus: true }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("input", { className: "smf-input smf-input-desc", placeholder: t2.stream_marker_desc, value: newDesc, onChange: (e) => setNewDesc(e.target.value), onKeyDown: (e) => e.key === "Enter" && addMarker() }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "smf-btns", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "smf-save", onClick: addMarker, disabled: !newName.trim(), children: t2.stream_marker_save }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "smf-cancel", onClick: () => {
              setShowAddForm(false);
              setNewName("");
              setNewDesc("");
            }, children: t2.stream_marker_cancel })
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "stream-markers-list", children: [
          persistedMarkers.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "stream-no-markers", children: t2.stream_no_markers }),
          [...persistedMarkers].reverse().map((m2) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "stream-marker-item", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "smi-left", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "smi-dot" }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "smi-info", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "smi-name", children: m2.name }),
                m2.description && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "smi-desc", children: m2.description }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "smi-time", children: [
                  t2.stream_at,
                  " ",
                  secsToTimestamp(m2.streamPos)
                ] })
              ] })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "smi-actions", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "smi-cut", onClick: () => cutFromMarker(m2), disabled: streamPos <= m2.streamPos, title: t2.stream_cut_from, children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { width: "11", height: "11", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("path", { d: "M12 16l-4-4h2.5V4h3v8H16l-4 4z" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("path", { d: "M20 18H4" })
                ] }),
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "smi-del", onClick: () => deleteMarker(m2.id), title: t2.stream_marker_delete, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { width: "11", height: "11", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("polyline", { points: "3 6 5 6 21 6" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("path", { d: "M19 6l-1 14H6L5 6" })
              ] }) })
            ] })
...
function App() {
  const [view, setView] = reactExports.useState("download");
  ...
  const [fetchErr, setFetchErr] = reactExports.useState("");
  const [ready, setReady] = reactExports.useState(false);
  ...
  const handleThemeChange = (th2) => {
    setTheme(th2);
    saveState({ theme: th2 });
    document.documentElement.setAttribute("data-theme", th2 === "apathy" ? "apathy" : "");
  };
  reactExports.useEffect(() => {
    (async () => {
  ...
  const handleSetup = async () => {
    ...
    setSetupError("");
    try {
      const r2 = await window.api.setupYtDlp();
      if (r2.success) {
        setReady(true);
        setShowSetup(false);
  ...
  const handleFetch = reactExports.useCallback(async (url) => {
    ...
    if (url.includes("music.youtube.com")) setInitFmt((p2) => ({ ...p2, type: "audio" }));
    else if (url.includes("youtube.com") || url.includes("youtu.be")) setInitFmt((p2) => ({ ...p2, type: "video" }));
  ...
  const handleDownload = reactExports.useCallback(async (type, quality, timeRange) => {
    if (!videoInfo || !window.api) return;
    const id2 = genId();
    const isTwitch = platform === "twitch";
    let formatArgs = isTwitch ? getTwitchFormatArgs(type, quality) : getFormatArgs(type, quality);
    let sectionDuration;
    if (timeRange) {
    ...
    const formatLabel = timeRange ? `${baseLabel} [${secsToTimestamp(timeRange.start)}→${timeRange.end >= 0 ? secsToTimestamp(timeRange.end) : "end"}]` : baseLabel;
    ...
    const r2 = await window.api.startDownload({ id: id2, url: downloadUrl, formatArgs, downloadPath: settings.downloadPath, sectionDuration });
  ...
  const handleFfmpegHint = reactExports.useCallback(async () => {
    const id2 = `ffmpeg_install_${Date.now()}`;
    setDownloads((p2) => [{ id: id2, url: "", title: "Installing ffmpeg...", formatLabel: "System", status: "downloading", progress: 0, createdAt: Date.now() }, ...p2]);
    const unsub = window.api?.onFfmpegDownloadProgress?.((d) => {
      setDownloads((p2) => p2.map((x2) => x2.id === id2 ? { ...x2, title: `ffmpeg: ${d.step}` } : x2));
    });
    const r2 = await window.api?.downloadFfmpeg();
    unsub?.();
    if (r2?.success) {
      setDownloads((p2) => p2.map((x2) => x2.id === id2 ? { ...x2, status: "complete", title: "ffmpeg installed ✓", progress: 100 } : x2));
    } else {
      setDownloads((p2) => p2.map((x2) => x2.id === id2 ? { ...x2, status: "error", title: "ffmpeg install failed", error: r2?.error ?? "Unknown error" } : x2));
    }
  }, []);
  const handleCancel = reactExports.useCallback(async (id2) => {
    await window.api?.cancelDownload(id2);
    setDownloads((p2) => {
      const updated = p2.map((x2) => x2.id === id2 ? { ...x2, status: "cancelled" } : x2);
      const item = updated.find((x2) => x2.id === id2);
  ...
  const handlePickFolder = async () => {
    const p2 = await window.api?.selectDownloadFolder();
    if (p2) {
      setSettings((prev) => ({ ...prev, downloadPath: p2 }));
      await saveState({ downloadPath: p2 });
    }
  };
        ...
        view === "download" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dl-view", children: [
          ...
          downloads.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "queue-section", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "queue-head", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "section-eyebrow-sm", children: t2.lbl_downloads }),
              activeCount > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "queue-badge", children: [
                activeCount,
                " ",
                t2.lbl_active
              ] })

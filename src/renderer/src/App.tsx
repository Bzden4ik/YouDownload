...
interface VideoInfo { id: string; title: string; thumbnail?: string; duration?: number; view_count?: number; uploader?: string; description?: string; webpage_url?: string }
...
function getFormatLabel(type: FormatType, quality: VideoQuality | AudioQuality): string {
  ...
  const m: Record<VideoQuality,string> = { '1080':'1080p · FHD','720':'720p · HD','480':'480p · SD','360':'360p','240':'240p','144':'144p' }
...
function FormatSelector({ onDownload, onDownloadAll, disabled, t, initType, initVq, initAq, onFormatChange, playlist, platform }: {
  ...
  const VQ = [{ v:'1080' as VideoQuality, l:'1080p', b:'FHD' },{ v:'720' as VideoQuality, l:'720p', b:'HD' },{ v:'480' as VideoQuality, l:'480p', b:'SD' },{ v:'360' as VideoQuality, l:'360p', b:'' },{ v:'240' as VideoQuality, l:'240p', b:'' },{ v:'144' as VideoQuality, l:'144p', b:'' }]
  ...
  const ql = isTwitch ? (ftype==='video'?TQ.find(q=>q.v===tq)?.l:'MP3') : (ftype==='video'?VQ.find(q=>q.v===vq)?.l:AQ.find(q=>q.v===aq)?.s)
    ...
    <div className="fmt-section">
      ...
      <div className="quality-grid">
        {ftype==='audio' ? AQ.map(q=><button key={q.v} className={`q-btn ${aq===q.v?'q-btn-on':''}`} onClick={()=>setAqAndSave(q.v)}><span className="q-main">{q.f}</span><span className="q-sub">{q.s}</span></button>)
         ...
         : VQ.map(q=><button key={q.v} className={`q-btn ${vq===q.v?'q-btn-on':''}`} onClick={()=>setVqAndSave(q.v)}><span className="q-main">{q.l}</span>{q.b&&<span className="q-badge">{q.b}</span>}</button>)}
      </div>
        ...
        <main className="main">
          {view==='download' && (
            <div className="dl-view">
              ...
              {fetchErr && <div className="fetch-err"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>{fetchErr}</div>}
              ...
              {(videoInfo || fetching) && (
                <>
                  ...
                  <FormatSelector key="fmt-selector" onDownload={handleDownload} onDownloadAll={handleDownloadAll} playlist={playlist} platform={platform} disabled={!videoInfo||fetching} t={t} initType={initFmt.type} initVq={initFmt.vq} initAq={initFmt.aq} onFormatChange={(type,vq,aq)=>saveState({formatType:type,videoQuality:vq,audioQuality:aq})}/>
                </>
              )}
